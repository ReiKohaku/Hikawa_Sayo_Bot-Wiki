...
`-p 页码`与[搜索官方歌曲（谱面）](#搜索官方歌曲（谱面）)中的同名参数一致。
...
`谱面id`是您所要查询的谱面的ID，有两种方式查询：1. 对于官方谱面来说，您可以在Bestdori!对应的歌曲详情页面，看到该歌曲的ID；对于自制谱面来说，您需要打开对应谱面详情页面，然后观察地址栏，其格式应该为`bestdori.com/community/charts/xxx...`，其中的`xxx`即为该谱面的ID，它应当是一个正整数；2. 使用[搜索官方歌曲（谱面）](#搜索官方歌曲（谱面）)或[搜索自制谱面](#搜索自制谱面)查询。
...
`谱面id`与[查询谱面基本信息](#查询谱面基本信息)中的同名参数一致。
